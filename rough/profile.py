...
# %% ..\01_profile.ipynb 7
def Sk(im, #Numpy array or array like
         axis = 1, #Default to Skew of rows
         norm = True #Normalize the profile by subtracting the mean
        ):
    '''
    Calcultes the Skew (Sk) along given axis. Thin wrapper around scipy.stats.skew with bias set to False
    '''
    if norm:
        im = im - np.mean(im, axis=axis, keepdims=True)
    return skew(im, axis=axis)
